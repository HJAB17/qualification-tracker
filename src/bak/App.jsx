...
const App = () => {
  ...
  const getQualificationsForEnvironment = (environmentName) => {
    return qualifications.filter(q => 
      ...
      (searchTerm === "" || q.name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  };
  ...
  const exportData = (format) => {
    if (format === 'csv') {
      const headers = ['Nom', 'Début', 'Fin', 'Environnement', 'Déployé'];
      const csvContent = [
        headers.join(','),
        ...qualifications.map(q => 
          `${q.name},${q.startDate.toISOString().split('T')[0]},${q.endDate.toISOString().split('T')[0]},${q.environment},${q.deployed}`
        )
      ].join('\n');
                    ...
                    <div className="flex items-start">
                        ...
                        {sortedQualifications.length === 0 ? (
                          ...
                          <div className="space-y-1">
                            {sortedQualifications.map((qual, index) => (
                              <div 
                                key={qual.id}
                                className="h-12 mb-1 flex items-center cursor-pointer hover:bg-gray-50 rounded-md transition-colors border-l-4"
                      ...
                      <div className="flex-1 min-w-0 relative">
                        {sortedQualifications.length === 0 ? (
                          ...
                          <div className="space-y-1">
                            {sortedQualifications.map((qual, index) => (
                              <div key={qual.id} className="h-12 mb-1 relative">
        ...
        <div className="bg-white shadow-sm rounded-lg p-6">
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <ul className="text-sm text-gray-600 space-y-1">
              ...
              <li>• Les noms des versions sont affichés à gauche avec leurs dates</li>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="px-6 py-4">
              <p className="text-sm text-gray-600">
                Êtes-vous sûr de vouloir supprimer la version <strong>"{qualificationToDelete.name}"</strong> ?
              </p>
